...
function preload(){
  obs1Img=loadImage("a.png")
  obs2Img =loadImage("b.png")
  obs3Img =loadImage("e.png")
...
if(doorsGroup.isTouching(bulletGroup)||climbersGroup.isTouching(bulletGroup)||invisibleBlockGroup.isTouching(bulletGroup)){
  for(var i=0;i<doorsGroup.length;i++){     
      
   if(doorsGroup.isTouching(bulletGroup)||climbersGroup.isTouching(bulletGroup)||invisibleBlockGroup.isTouching(bulletGroup)){
        doorsGroup[i].destroy()
        climbersGroup[i].destroy()
        invisibleBlockGroup[i].destroy()
...
/*function enemy(){
  if(frameCount%50===0){
    ...
    zombie = createSprite(random(10,500),random(10,100),40,40)
...
if (frameCount % 240 === 0) {
  ...
  climber.x = Math.round(random(200,400));
  invisibleBlock.x = Math.round(random(300,400));
  
  door.addImage(obs1Img);
  climber.addImage(obs2Img);
  invisibleBlock.addImage(obs3Img);
  door.velocityY = 1;
  climber.velocityY = 1;
